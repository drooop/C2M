...
class GetUserInfoFirstStep(tornado.web.RequestHandler):
    def get(self):
        global REDIRECT_URI
        # 查看是否vip用户
        drop = self.get_query_arguments('drop')
        if drop:
            REDIRECT_URI += '?drop=true'
        ...
        # 还原REDIRECT
        REDIRECT_URI = 'http://c2m.tq.yhlcps.com/backend/getUserInfo'
...
class GetUserInfoSecondStep(tornado.web.RequestHandler):
    def get(self):
        ...
        access_token_url = source_url.format(
        ...
        print(userinfo)
        # 准备重定向参数
        # self._url = 'http://c2m.tq.yhlcps.com/#/login/'+openid
        if drop:
            self._url = 'http://c2m.tq.yhlcps.com/#/vip_ad/'+openid
        else:
            self._url = 'http://c2m.tq.yhlcps.com/#/ad/'+openid
        self._permanent = True
...
class HandleWeiXinMessage(tornado.web.RequestHandler):
    ...
    def post(self):
        ...
        if isinstance(recMsg, recieve.Msg) and recMsg.MsgType == 'text':
            ...
            content = grt.producer(toUser, recMsg.Content)
            # content = recMsg.Content

            # content = "欢迎关注云海流工业互联网平台系统！您发送的文字我们收到了哟～"
            #  + str(recMsg.Content)

            if recMsg.Content == '取件码':
                content = '点击下面的链接查看该订单的制作视频\nhttp://c2m.tq.yhlcps.com/statics/video/TPRecord_2020-06-23-015631.mp4'
            ...
            res = str(render.reply_text(
                toUser, fromUser, int(time.time()), content))
